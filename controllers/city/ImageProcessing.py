...
def classify_and_measure_distances_new(lines, image_width):
    ...
    if (not left_distances) and right_distances:
        left_distance = lane_width_pixel - min(previous_right_distance) if previous_right_distance else 250
        # left_distance = 250  # No left line, default fallback
    elif left_distances:
        left_distance = min(left_distances)
        previous_left_distance.append(left_distance)
    elif previous_left_distance:
    ...
    if (not right_distances) and left_distances:
        # right_distance = 250  # No right line, default fallback
        right_distance = lane_width_pixel - min(previous_left_distance) if previous_left_distance else 250
